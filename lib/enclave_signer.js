...
import { utf8ToBytes, concatBytes, hexToBytes, } from "@noble/curves/abstract/utils";
const isUND = (val) => {
...
export async function pwhash(passwd, user_info, key_length_bytes) {
    ...
    const raw_pwd = await EnclaveSigner.hsm().importKey("raw", new TextEncoder().encode(pwd_pt), {
        name: "PBKDF2",
    }, true, ["deriveBits"]);
