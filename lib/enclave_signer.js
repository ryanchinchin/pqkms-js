...
export default class UserRegistrationManager {
    ...
    async registerUser(raw_pw, user_info, signing_priv, progress) {
        ...
        let signed = await this.signEnclaves(signing_priv, user_info.mrsigner_pub, enclaves);
...
export async function register_user(domain, email_addr, password, base_url, crypto_key) {
    ...
    const { privateKey, publicKey } = crypto_key || (await new EnclaveSigner().sgx_rsa_key());
    let user = {
        domain_name: domain,
