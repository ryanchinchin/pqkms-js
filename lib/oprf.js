...
export class OprfClient {
    ...
    async finalize(evaluatedElement, clientData) {
        ...
        let hashInput = concatBytes(numberToBytesBE(clientData.hashed_password.length, 2), toUtf8Bytes(clientData.hashed_password), numberToBytesBE(final_point.length, 2), final_point, toUtf8Bytes("Finalize"));
        let digest = await globalThis.crypto.subtle.digest("SHA-512", hashInput.buffer);
        return new Uint8Array(digest);
    }
    async login_key(session_key, hashed_pw) {
        const CURVE = this.EcGroup.CURVE;
        const ProjectivePoint = this.EcGroup.ProjectivePoint;
        const crypto = globalThis.crypto.subtle;
        const salt = concatBytes(toUtf8Bytes(hashed_pw), toUtf8Bytes("LoginInfoSalt"));
        const info = toUtf8Bytes("LoginInfo");
        let privateKeyInp = bytesToNumberBE(hkdf(sha512, session_key, salt, info, 512 / 8));
    ...
    async lockbox_key(session_key, hashed_pw) {
        const crypto = globalThis.crypto.subtle;
