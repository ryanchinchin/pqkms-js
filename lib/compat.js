import { assert, b64urlDecode, b64urlEncode, bytesToNumberBE, numberToVarBytesBE, } from "./utils";
import { mod, pow } from "@noble/curves/abstract/modular";
export function xgcd(a, b) {
    let r0;
    let r1;
    let s0 = 1n;
    let s1 = 0n;
...
async function genSgxKeyFromPrimes(subtle, p, q) {
    const n = p * q;
    const phi = (p - 1n) * (q - 1n);
    const e = 3n;
    ...
    const privKey = await subtle.importKey("jwk", k, {
        name: "RSASSA-PKCS1-v1_5",
        hash: "SHA-256",
    }, true, ["sign"]);
    const pubKey = await subtle.importKey("jwk", k_pub, {
        name: "RSASSA-PKCS1-v1_5",
        hash: "SHA-256",
    }, true, ["verify"]);
    return { privateKey: privKey, publicKey: pubKey };
}
export async function sgxCompatKey(subtle) {
    let count = 0;
